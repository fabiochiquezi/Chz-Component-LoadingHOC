...
export const LoadingHOC: ILoadingHOC = ({ condition, children, loading, animChildren, callBack }) => {
  ...
  return (
    <Div>
      <div ref={loadingRef} className="display-none fade-out" data-testid="loading-HOC">{loading ?? <LoadingPage />}</div>
      <div ref={childrenRef} className="display-none fade-out" id="children-HOC">{children}</div>
    </Div>
  )
}
